...
const ProductContextProvider = ({children}) => {
    ...
    const getProducts = async () =>{
        ...
        const res = await axios(`${JSON_API_PRODUCTS}?_limit=3&${window.location.search}`)
        dispatch({
            type: "GET_PRODUCTS",
            payload: res
        })
    ...
    const addProduct =(newProduct)=>{
        axios.post(JSON_API_PRODUCTS, newProduct)
        getProducts()
    }

    const addProductToFav=(newFavProduct)=>{
      axios.post(JSON_API_PRODUCTS, newFavProduct);
      getProducts();
    }
    ...
    const getCart = () => {
      ...
      const addProductToCart = (product) => {
        ...
        const addProductToFavourites = (product)=>{
          ...
          productToFind=favourites.products.filter(
            (item) => item.item.id===product.id
          )

          if(productToFind.length==0){
            favourites.products.push(newFavProduct);
          }
        }
